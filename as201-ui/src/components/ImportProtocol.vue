  ...
  <div class="import-model">
    <div class="import-div">
      ...
      <div class="path">
        ...
        <img src="../images/users/return_path.png"
          v-if="this.$store.getters.user.group=='admin'&&protocols_users=='protocols'" @click="backAll" />
      </div>
      <el-table :data="import_protocols" class="export-users" key="t1"
      ...
      <div class="users-table" v-else>
        <div class="users-title">
          <div style="margin-left: 78px;">{{$t("language.name")}}</div>
        </div>
        <div v-for="(item,index) in users" :key="index" class="users-content" @click="getUserPro(item)">
          <div class="img-folder">
            <img src="../images/system/folder.png" />
          </div>
      ...
      <div class="total-selected">
        <div class="total">{{this.$t("language.total")}}{{import_protocols.length}}</div>
  ...
  export default {
    ...
    mounted() {
      ...
      this.EventBus.on(this.Notify.CODE_GET_FILES_SUCCESS, (notify) => {
        this.import_protocols = notify.Data
        for (var i = 0; i < this.import_protocols.length; i++) {
          this.import_protocols[i].time = moment(this.import_protocols[i].updated_at * 1000).format(
            'MM/DD/YYYY HH:mm:ss')
        }
      })
    },
    methods: {
      ...
      getPath() {
        this.path = "U disk > Leap-Pure S24 > Protocols > " + this.$store.getters.user.username
        getUserProtocols().then((res) => {
        })
      },
      cancleImport() {
        this.$emit('cancleImport')
      ...
      getUserPro(item) {
        // this.path = "U disk > Leap-Pure S24 > Protocols > " + item.name
        getUserProtocols({
          'user_directory_name': item.name
        }).then((res) => {
          if (res.code == 0) {
            this.protocols_users = 'protocols'
          }
        })
      }
    }
